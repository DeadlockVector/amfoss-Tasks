...
fn main() {
    ...
    let mut writing_handler = csv::Writer::from_path("crypto.csv").unwrap();

    writing_handler.write_record(&["Crypto Name" , "Price" , "24H Change" , "24H Volume" , "Market cap"]).unwrap();
    ...
    for element in document.select(&_crypto_selector) {
        ...
        writing_handler.write_record([&crypto_name , &crypto_price , &crypto_24hourchange , &crypto_24hourvolume , &crypto_marketcap]).expect("could not create selector");
    }

    writing_handler.flush().expect("could not close file");
    println!("done");
